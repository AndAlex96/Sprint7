...
class TestLoginCourier:
    ...
    def test_courier_is_login_passed(self, register_new_courier_and_return_login_password):
        ...
        response_auth = requests.post('https://qa-scooter.praktikum-services.ru/api/v1/courier/login', data=payload_login_password)
        assert response_auth.status_code == 200
        assert 'id' in response_auth.json()

    @allure.title('Проверка появления ошибки при авторизации с неверным логином')
    def test_error_auth_with_incorrect_login(self, register_new_courier_and_return_login_password):
        ...
        response_auth = requests.post('https://qa-scooter.praktikum-services.ru/api/v1/courier/login',
                                      data=new_payload)
        assert response_auth.status_code == 404

    @allure.title('Проверка появления ошибки при авторизации с неверным паролем')
    ...
    @allure.title('Проверка появления ошибки при попытки авторизации с незарегистрированными данными')
