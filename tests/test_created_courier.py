...
from app.helpers.generate_random_string import generate_random_string
...
class TestCreatingCourier:
    ...
    @allure.step('Проверка невозможности создания идентичного курьера')
    ...
    @allure.step('Проверка невозможности создания курьера без логина')
    ...
    @allure.step('Проверка невозможности создания курьера без пароля')
    ...
    @allure.step('Проверка невозможности создания курьера с уже существующим логином')
    def test_error_create_couriers_with_created_login(self, register_new_courier_and_return_login_password, generate_random_string):
        login, password, first_name = register_new_courier_and_return_login_password
        payload = {'login': login, 'password': password, 'firstName': first_name}
        ...
        response = requests.post(f'{BASE_URL}/api/v1/courier', data=payload2)
